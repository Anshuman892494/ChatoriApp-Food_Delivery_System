...
const initForm = { name: '', price: '', category: '', isAvailable: true, image: null };
...
const Foods = () => {
    ...
    const fetchAll = async () => {
        const [foodsRes, restRes] = await Promise.all([
            api.get(`/admin/restaurants/${restaurantId}/foods`),
            api.get('/admin/restaurants'),
        ]);
        setFoods(foodsRes.data);
        setRestaurant(restRes.data.find(r => r._id === restaurantId));
        setLoading(false);
    };
    useEffect(() => { fetchAll(); }, [restaurantId]);

    const openCreate = () => { setEditing(null); setForm(initForm); setShowModal(true); };
    const openEdit = (f) => { setEditing(f); setForm({ name: f.name, price: f.price, category: f.category, isAvailable: f.isAvailable, image: null }); setShowModal(true); };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fd.append('category', form.category); fd.append('isAvailable', form.isAvailable);
            if (form.image) fd.append('image', form.image);
            if (editing) {
                await api.put(`/admin/foods/${editing._id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
                toast.success('Food item updated');
            } else {
                await api.post(`/admin/restaurants/${restaurantId}/foods`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        ...
        <AdminLayout>
            <div className="mb-4">
                <Link to="/admin/restaurants"
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:border-orange-400 hover:text-orange-500 transition-all">
                    <FiArrowLeft size={14} /> Back
                </Link>
            </div>
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">{restaurant?.name} — Foods</h1>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-800/50">
                            <tr>{['Image', 'Name', 'Category', 'Price', 'Available', 'Actions'].map(h => (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setShowModal(false)}>
                    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
                            <h3 className="font-extrabold text-gray-900 dark:text-white">{editing ? 'Edit Food Item' : 'Add Food Item'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div><label className={labelCls}>Category</label><input className={inputCls} placeholder="e.g. Burgers, Pizza" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} required /></div>
